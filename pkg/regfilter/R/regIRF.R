...
###############################################################
#' @export
regIRF <- function(x, ...) UseMethod("regIRF")
...
#'
#' @seealso \code{\link{regIPF}}, \code{\link{regCVCF}}, \code{\link{regFMF}}, \code{\link{print.rfdata}}, \code{\link{summary.rfdata}}
#' @name regIRF
NULL
...
regIRF.default <- function(x, y, t=0.2, ...){
  ...
  while(KeepOn){
    counter <- counter+1
    model <- rpart(formu, dataset)
    pr_rpart <- predict(model, dataset[,-output], type = "vector")
    IndexesToRemove <- which(forecast(prediccion = pr_rpart, real = dataset[,output], t))

    if(length(IndexesToRemove)==0){
      KeepOn <- FALSE
    }else{
      dataset <- dataset[-IndexesToRemove,]
    }
  }
  ...
  ret <- list(xclean = xclean,
              ...
              numnoise = numnoise,
              idnoise = idnoise,
              filter = "Iterative Robust Filter",
              param = param,
              call = call)
  class(ret) <- "rfdata"
  return(ret)
...
regIRF.formula <- function(formula, data, ...){
  ...
  res <- regIRF.default(x = x, y = y, ...)
  res$call <- match.call(expand.dots = TRUE)
  res$call[[1]] <- as.name("regIRF")
